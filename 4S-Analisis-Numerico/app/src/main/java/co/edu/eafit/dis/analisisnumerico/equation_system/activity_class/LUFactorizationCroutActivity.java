...
import co.edu.eafit.dis.analisisnumerico.equation_system.method_class.LUFactorizationCrout;
...
public class LUFactorizationCroutActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lufactorization_crout);
    ...
    public void onCreateClick(View view){
        try {
            ...
            Toast.makeText(LUFactorizationCroutActivity.this,"Error, please check " +
    ...
    public void onCalculateClick(View view){
        try {
            getMatrixData();
            LUFactorizationCrout.crout(DataUtil.matrixA,DataUtil.matrixA.length);
            createViewResult();
        }catch (Exception e){
            Toast.makeText(LUFactorizationCroutActivity.this,"Error, please check " +
                    "the data",Toast.LENGTH_LONG).show();
    ...
    public void createViewResult() {
        ...
        for(int i=0;i<LUFactorizationCrout.tableArrayL.size()-1;i++){
            tableRow = new TableRow(this);
            for(int j=0;j<LUFactorizationCrout.tableArrayL.get(j).size();j++){
                textView = new TextView(this);
                textView.setPadding(5,5,8,5);
                textView.setGravity(Gravity.START);
                textView.setTextSize(15);
                textView.setText(LUFactorizationCrout.tableArrayL.get(i).get(j));
                tableRow.addView(textView);
            }
            tableL.addView(tableRow);
        }
        for(int i=0;i<LUFactorizationCrout.tableArrayU.size()-1;i++){
            tableRow = new TableRow(this);
            for(int j=0;j<LUFactorizationCrout.tableArrayU.get(j).size();j++){
                textView = new TextView(this);
                textView.setPadding(5,5,8,5);
                textView.setGravity(Gravity.START);
                textView.setTextSize(15);
                textView.setText(LUFactorizationCrout.tableArrayU.get(i).get(j));
                tableRow.addView(textView);
            }
                ...
                .setNeutralButton(R.string.partial, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        try {
                            ...
                            Toast.makeText(LUFactorizationCroutActivity.this,
