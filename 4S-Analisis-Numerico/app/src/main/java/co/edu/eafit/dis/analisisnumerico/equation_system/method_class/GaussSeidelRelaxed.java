...
public class GaussSeidelRelaxed {

    public static ArrayList<ArrayList<String>> tableArray = new ArrayList<ArrayList<String>>();

    public static BigDecimal[] gaussSeidelRelaxed(BigDecimal[][] mA, BigDecimal[] xVal, BigDecimal mb[],
                                                  int iter, BigDecimal tol, BigDecimal lambda,int n)
            throws Exception {
        tableArray.clear();
        ...
        while(error.compareTo(tol)>0 && count < iter){
            ...
            for(int i=0; i<n;i++){
                ...
                x[i]=(lambda.multiply(b[i].subtract(sum)).divide(A[i][i],32, RoundingMode.HALF_UP))
                        .add((BigDecimal.ONE.subtract(lambda)).multiply(x0[i]));
                aux=x[i].subtract(x0[i]);
                error = error.add(new BigDecimal(Math.sqrt(Math.abs(aux.doubleValue()))));
                x0[i]=x[i];
                tableArray.get(contArray).add(x0[i].toString());
            }
