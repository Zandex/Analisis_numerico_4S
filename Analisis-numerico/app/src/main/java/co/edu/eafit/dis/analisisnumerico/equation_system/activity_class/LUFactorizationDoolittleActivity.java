...
import co.edu.eafit.dis.analisisnumerico.equation_system.method_class.LUFactorizationDoolittle;
...
public class LUFactorizationDoolittleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lufactorization_doolittle);
        ...
        try {
            ...
            Toast.makeText(this,"Error, something happened " +
                    "creating the view",Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onCreateClick(View view){
        try {
            ...
            Toast.makeText(LUFactorizationDoolittleActivity.this,"Error, please check " +
    ...
    public void onCalculateClick(View view){
        try {
            getMatrixData();
            LUFactorizationDoolittle.doolittle(DataUtil.matrixA,DataUtil.matrixA.length);
            createViewResult();
        }catch (Exception e){
            Toast.makeText(LUFactorizationDoolittleActivity.this,"Error, please check " +
                    "the data",Toast.LENGTH_LONG).show();
    ...
    public void createViewResult() {
        ...
        for(int i=0;i<LUFactorizationDoolittle.tableArrayL.size()-1;i++){
            tableRow = new TableRow(this);
            for(int j=0;j<LUFactorizationDoolittle.tableArrayL.get(j).size();j++){
                textView = new TextView(this);
                textView.setPadding(5,5,8,5);
                textView.setGravity(Gravity.START);
                textView.setTextSize(15);
                textView.setText(LUFactorizationDoolittle.tableArrayL.get(i).get(j));
                tableRow.addView(textView);
            }
            tableL.addView(tableRow);
        }
        for(int i=0;i<LUFactorizationDoolittle.tableArrayU.size()-1;i++){
            tableRow = new TableRow(this);
            for(int j=0;j<LUFactorizationDoolittle.tableArrayU.get(j).size();j++){
                textView = new TextView(this);
                textView.setPadding(5,5,8,5);
                textView.setGravity(Gravity.START);
                textView.setTextSize(15);
                textView.setText(LUFactorizationDoolittle.tableArrayU.get(i).get(j));
                tableRow.addView(textView);
            }
                ...
                .setNeutralButton(R.string.partial, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        try {
                            ...
                            Toast.makeText(LUFactorizationDoolittleActivity.this,
