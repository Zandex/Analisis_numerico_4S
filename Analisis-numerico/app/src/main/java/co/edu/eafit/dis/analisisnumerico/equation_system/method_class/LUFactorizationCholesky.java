...
public class LUFactorizationCholesky {
    ...
    public static ArrayList<ArrayList<String>> tableArrayU = new ArrayList<ArrayList<String>>();
        ...
        public static void cholesky(BigDecimal[][]mA, int n)throws Exception{
            ...
            for(int k=0; k<n;k++){
                ...
                L[k][k] = new BigDecimal(Math.sqrt(A[k][k].doubleValue()));
                U[k][k]= L[k][k];
                for(int p=0; p<k;p++) {
                    sum = sum.add(L[k][p].multiply(U[p][k]));
                }
                L[k][k] = new BigDecimal(Math.sqrt(A[k][k].subtract(sum).doubleValue()));
                U[k][k]= L[k][k];
                contArrayL = printViewDataL(L,n,contArrayL);
                contArrayU = printViewDataU(U,n,contArrayU);
                for(int i=k+1; i<n;i++){
                    ...
                    L[i][k]=(A[i][k].subtract(sum)).divide(L[k][k],32, RoundingMode.HALF_UP);
                    contArrayL = printViewDataL(L,n,contArrayL);
                }
                for(int j=k+1; j<n;j++){
                    sum=BigDecimal.ZERO;
