...
import co.edu.eafit.dis.analisisnumerico.interpolation.method_class.NewtonDividedDifference;
import co.edu.eafit.dis.analisisnumerico.utility_class.DataUtil;

public class NewtonDividedDifferenceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_newton_divided_difference);
    ...
    public void onCreateClick(View view){
        try {
            ...
            Toast.makeText(NewtonDividedDifferenceActivity.this,"Error, please check " +
    ...
    public void onCalculateClick(View view){
        try {
            getVectorsData();
            int result = NewtonDividedDifference.newtonDivided(DataUtil.funcFInterpolation,
                    DataUtil.vectorXInterpolation,
                    new BigDecimal(DataUtil.interpolation.get("Y_Value")),
                    DataUtil.vectorXInterpolation.length);
            createViewResult(new BigDecimal(result));
        }catch (Exception e){
            Toast.makeText(NewtonDividedDifferenceActivity.this,"Error, please check " +
                    "the data",Toast.LENGTH_LONG).show();
        }
    ...
    public void createViewResult(BigDecimal result){
        ...
        for(int i = 0; i< NewtonDividedDifference.tableArray.size()-1; i++){
            tableRow = new TableRow(this);
            for(int j=0;j<NewtonDividedDifference.tableArray.get(j).size();j++){
                textView = new TextView(this);
                textView.setPadding(5,5,8,5);
                textView.setGravity(Gravity.START);
                textView.setTextSize(15);
                textView.setText(NewtonDividedDifference.tableArray.get(i).get(j));
